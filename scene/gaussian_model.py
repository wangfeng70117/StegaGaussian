...
class GaussianModel:
    ...
    def restore(self, model_args, training_args, ft_watermark=False):
        if ft_watermark:
            print(f'正在加载预训练3DGS模型...')
            (self.active_sh_degree,
             self._xyz,
             self._features_dc,
             self._features_rest,
    ...
    def training_watermark_setup(self, training_args):
        l = [
            {'params': [self._features_dc], 'lr': 0.005, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': 0.005 / 20.0, "name": "f_rest"},
        ]
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
    ...
    def set_position(self, points):
        ...
        self._xyz = nn.Parameter(set_requires_grad(points.detach(), False))
